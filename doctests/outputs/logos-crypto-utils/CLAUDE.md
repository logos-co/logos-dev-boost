...
- **Name:** crypto_utils
- **Type:** Logos Module (core)
- **Interface:** Universal (pure C++ impl, generated Qt glue)
- **Description:** Logos module
...
This is a universal C++ module (a header-first cdylib). The build pipeline is:
1. `mkLogosModule` runs the universal codegen automatically (no `preConfigure`)
2. Generator derives `generated_code/<name>.lidl` from the pure C++ impl header
3. Generator produces `generated_code/*_cdylib_glue.{h,cpp}` (Qt-plugin glue) and `*_module_impl.cpp` (Qt-free C-ABI export wrapper)
4. CMake globs `generated_code/` and compiles everything into a Qt plugin `.so`/`.dylib`

Your code is pure C++ (std::string, int64_t, etc). The generator handles all Qt types.
...
logoscore call crypto_utils methodName args
logoscore stop

# Inspect module metadata and methods
lm ./result/lib/crypto_utils_plugin.so
lm methods ./result/lib/crypto_utils_plugin.so --json
```
...
- **create-full-app** — Create a Logos full app — a combined module + UI app project in a single root directory
