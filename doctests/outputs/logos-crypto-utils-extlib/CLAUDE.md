...
This is a universal C++ module. The build pipeline is:
1. `preConfigure` in flake.nix runs `logos-cpp-generator --from-header`
2. Generator reads the pure C++ impl header and metadata.json
3. Generator produces `generated_code/*_qt_glue.h` and `*_dispatch.cpp`
4. CMake compiles everything into a Qt plugin `.so`/`.dylib`
...
logoscore -m ./result/lib -l crypto_utils -c "crypto_utils.methodName(args)"
